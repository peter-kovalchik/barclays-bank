  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if(response?.status === 200 && response?.ok){
      ...
      if(authUser[0]?.length){
        const {name, email, password, avatar, total_income, total_spending, total_transactions, status} = authUser[0]
        ...
        <div className="box bg-primary/5 dark:bg-bg3 lg:p-6 xl:p-8 border border-n30 dark:border-n500">
          ...
          <div className="mt-8 flex gap-6">
            <button disabled={loading} type="submit" className="btn px-5">
              Login
              {loading && (
                <FontAwesomeIcon className="animate-spin rounded-full" icon={faSpinner} />
              )}
            </button>
          </div>
        </div>
