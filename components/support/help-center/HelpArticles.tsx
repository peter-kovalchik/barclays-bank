...
const HelpArticles = () => {
  ...
  const {
    ...
    endIndex,
    totalPages,
  } = usePagination(articles.length, itemsPerPage);

  const displayedData = articles.slice(startIndex, endIndex + 1);

  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    const searchTerm = e.target.value.toLowerCase();
    const result = articleList.filter(
      (item) =>
      ...
      <div className="flex justify-between flex-wrap items-center gap-4 bb-dashed mb-4 pb-4 lg:mb-6 lg:pb-6">
        ...
        <div className="flex items-center gap-4 flex-wrap grow sm:justify-end">
          <SearchBar
            ...
            classes="bg-primary/5 dark:bg-bg3"
          />
